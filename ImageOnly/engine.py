...
def train(model, dataloader, loss_fn, optimizer, device):
    ...
    for images, labels in dataloader:
        ...
        all_train_preds.append(torch.softmax(y_pred, dim=1).detach().cpu().numpy())
...
def eval(model, dataloader, loss_fn, device):
    ...
    with torch.inference_mode():
        for images, labels in dataloader:
            ...
            all_val_preds.append(torch.softmax(y_pred, dim=1).detach().cpu().numpy())
...
def cross_validate_model(
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
        ...
        model = ImageOnlyModel(num_classes=num_classes, backbone = backbone)
...
def retrain_on_full_data(
    ...
    for epoch in range(epochs):
        ...
        # Early stopping logic (requires any metric to improve)
