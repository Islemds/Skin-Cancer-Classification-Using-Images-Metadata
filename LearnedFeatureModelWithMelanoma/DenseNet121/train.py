...
results_df.to_csv(f"CV_{FUSION_OPERATION}_results_densenet121_N_SPLITS={N_SPLITS}_Epochs={NUM_EPOCHS_CROSS_VALIDATION}_batch={BATCH_SIZE}_lr={LEARNING_RATE}_melanoma.csv", index=False)
print(f"\nResults saved")
...
target_dir = "models"
model_name = f"{FUSION_OPERATION}_densenet121_N_Epochs={NUM_EPOCHS}_batch={BATCH_SIZE}_lr={LEARNING_RATE}_melanoma.pth"
...
metrics_df.to_csv(f"{FUSION_OPERATION}_results_densenet121_N_Epochs={NUM_EPOCHS}_batch={BATCH_SIZE}_lr={LEARNING_RATE}_melanoma.csv", index=False)
print(f"\nMetrics saved to")
